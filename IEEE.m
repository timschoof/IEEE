function correct = IEEE(varargin)
...
% y=IEEE(3,KeyWords{2},list,sentence,Fs,['supremes.wav' or level of difficulty], y])
...
% IEEE Application M-file for IEEE.fig
%    FIG = IEEE launch IEEE GUI.
%    IEEE('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 12-Dec-2005 08:07:18
...
if nargin == 0 | isnumeric(varargin{1}) % LAUNCH GUI
   ...
   set(handles.togglebutton5, 'String', KeyWords{5});
   if (varargin{5}==0)
   ...
   set(handles.togglebutton5, 'ForegroundColor', [1 0 0]);
   set(handles.togglebutton5, 'FontWeight', 'light');
   set(handles.togglebutton5, 'Value', 0);
   AddUpButtons(handles);
   pause(0.3)
   if (varargin{5}==0)
       [y,Fs] = audioread(['',varargin{6},'.wav','']);
       if varargin{8} == 1 % if you're using playrec
           playrec('play', y, [3,4]);
       elseif varargin{8} == 0 % if you're not using playrec
           aud_obj = audioplayer(y,Fs);
   ...
   if ~ishandle(fig)
      ...
      correct(5) = get(handles.togglebutton5, 'Value');
%	   delete(fig);
   end
...
if (get(handles.togglebutton5, 'Value'))
   set(handles.togglebutton5, 'ForegroundColor', [0 0 0]);
   set(handles.togglebutton5, 'FontWeight', 'bold');
else 
   set(handles.togglebutton5, 'ForegroundColor', [1 0 0]);
   set(handles.togglebutton5, 'FontWeight', 'light');
end
...
set(handles.togglebutton5, 'ForegroundColor', [0 0 0]);
set(handles.togglebutton5, 'FontWeight', 'bold');
set(handles.togglebutton5, 'Value', 1.0);
AddUpButtons(handles);
...
function AddUpButtons(handles)
   set(handles.TotalBox, 'String', sprintf('Total= %d', ...
      ...
      +get(handles.togglebutton5, 'Value')...
  ));
